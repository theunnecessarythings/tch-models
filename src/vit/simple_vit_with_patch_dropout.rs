...
fn posemb_sincos_2d(patches: &Tensor, temperature: i64, dtype: Kind) -> Tensor {
    let (_, h, w, dim) = patches.size4().unwrap();
...
pub fn simple_vit_with_patch_dropout(
    p: &nn::Path,
    image_size: (i64, i64),
    patch_size: (i64, i64),
    num_classes: i64,
    ...
    nn::func_t(move |xs, train| {
        ...
        ys = ys.view([b, -1, dim]) + posemb_sincos_2d(&ys, 10000, Kind::Float);
        ys.apply_t(&patch_dropout, train)
