...
pub fn simple_vit_with_register_tokens(
    p: &nn::Path,
    image_size: (i64, i64),
    patch_size: (i64, i64),
    num_classes: i64,
    ...
    let register_tokens = p.var(
        "register_tokens",
        &[num_register_tokens, dim],
        nn::Init::Randn {
            mean: 0.0,
            stdev: 1.0,
        },
    ...
    nn::func_t(move |xs, train| {
        ...
        ys += &pos_embedding;
        ys = Tensor::cat(&[ys, register_tokens.repeat([b, 1, 1])], 1).apply_t(&transformer, train);
        ys.i((.., 0, ..))
